...
void CreateDir_Iext(int nbpop, int nPrtr, double I, string &path) {
  ...
  case -1 :
    path = path + "/CosPrtr/Iext_" + spop + sI ;
    break ;
    
  case 0 :
    if(IF_WEAK)
      path = path + "/DeltaPrtrWeak/Iext_" + spop + sI ;
    else 
  ...
  case 1 :
    if(IF_WEAK)
      path = path + "/GaussPrtrWeak/Iext_" + spop + sI ;
    else
      path = path + "/GaussPrtr/Iext_" + spop + sI ;
    break ;

  case 2 :
    path = path + "/ExpPrtr/Iext_" + spop + sI ;
    break ;
...
void CreateDir_Prop(int nbpop, int nPrtr, double I, double Cff, string &path) {
  ...
  char cProp[10] ;
  sprintf(cProp,"%0.2f",Cff) ; 
  string sProp = string(cProp) ; 
  ...
  string mkdirp = "mkdir -p " ; 
  if(IF_WEAKPROP)
    path = path + "/PropWeak"+ sProp + "/Iext_" + spop + sI ; 
  else
    path = path + "/Prop"+ sProp + "/Iext_" + spop + sI ;
...
void CreateDir_JabLoop(string &path,double Jab) {
  ...
  char cJe0[10] ;
  if(IF_M0) 
    sprintf(cJe0,"%0.4f",m0) ;    
  else 
    sprintf(cJe0,"%0.4f",Je0) ;    
  string sJe0 = string(cJe0) ;
  ...
  string strAy = popList[Ay] ;

  if(IF_M0)
    path += "/m0" + sJe0 + "J" + strAx + strAy + sJab ;   
  else 
    path += "/Je0" + sJe0 + "J" + strAx + strAy + sJab ; 
...
void nbNeurons(int nbpop, unsigned long &N, unsigned long* &nbN) {
  N = (unsigned long) N*nbPref ;
  
  /* nbN = new unsigned long [nbpop]() ; */
  nbN = (unsigned long *) malloc( nbpop * sizeof(unsigned long)) ; 
  ...
  if(IF_Nk) {
    if(nbpop==2) {
      nbN[0]= (unsigned long) (N*75./100.) ;
      nbN[1]= (unsigned long) (N*25./100.) ;
    }
    if(nbpop==3) {
      nbN[0]= (unsigned long) (N*70./100.) ;
      nbN[1]= (unsigned long) (N*15/100.) ;
      nbN[2]= (unsigned long) (N*15./100.) ;
    }
    if(nbpop==4) {
      nbN[0]= (unsigned long) (N*75./100.) ; 
      nbN[1]= (unsigned long) 6400 ;
      nbN[2]= (unsigned long) 6400 ;
      nbN[3]= (unsigned long) 6400 ;
    }
  }
...
void cptNeurons(int nbpop, unsigned long *Nk, unsigned long* &Cpt) {
  ...
  Cpt = (unsigned long *) malloc( (nbpop+1) * sizeof(unsigned long)) ; 
