...
int main(int argc, char *argv[]) {
  ...
  if(IF_LONG_RANGE) {
    cudaCheck(cudaMalloc((void **)&dev_conVecPtrLR, (unsigned long long) chunckSize * sizeof(float)));
    cudaCheck(cudaMalloc((void **)&dev_preFactorLR, (unsigned long long) nbpop * N_NEURONS * sizeof(float)));
  }
  ...
  if(IF_SHARED) 
    printf("with shared inputs ... \n") ; 

  if(IF_LONG_RANGE) 
    printf("with long range connections ... \n") ; 
  ...
  double *host_SigmaLR ; 
  if(IF_LONG_RANGE)
    cudaCheck(cudaMallocHost((void **)&host_SigmaLR,  nbpop * sizeof(double))); 
  ...
  double *host_DijLR ;
  if(IF_LONG_RANGE)
    cudaCheck(cudaMallocHost((void **)&host_DijLR,  nbpop * sizeof(double))) ; 

  if(IF_SPACE || IF_RING) {
    for(int j=0;j<nbpop*nbpop;j++) {
      host_Dij[j] = Dij[j] ; 
      if(IF_LONG_RANGE)
	host_DijLR[j] = DijLR[j] ; 
    }
  ...
  double *host_AUTA_p ;
  if(IF_AUTA) {
    cudaCheck(cudaMallocHost((void **)&host_AUTA_p,  nbpop * sizeof(double))); 
    for(int j=0;j<nbpop;j++) 
      host_AUTA_p[j] = AUTA_p[j] ; 
  }
  cudaPrintfInit();
  ...
  case random:
	...
	if(IF_AUTA) 
	  kernelGenConAuta<<<BlocksPerGrid, ThreadsPerBlock>>>(devStates, dev_conVecPtr, i, maxNeurons, nbN, host_AUTA_p) ; 
	else
	  if(IF_SHARED)
	    kernelGenConMatShared<<<BlocksPerGrid, ThreadsPerBlock>>>(devStates, dev_conVecPtr, i, maxNeurons, nbN) ; 
	  else
  ...
  case distDependent:
    ...
    if(IF_LONG_RANGE)
      initPreFactor<<<BlocksPerGrid, ThreadsPerBlock>>>(dev_preFactorLR);
    ...
    for(unsigned long i = 0; i < nChunks; i++) { 
      ...
      if(IF_LONG_RANGE)
	initConVec<<<BlocksPerGrid, ThreadsPerBlock>>>(dev_conVecPtrLR, i, maxNeurons) ; 
	...
	if(IF_LONG_RANGE)
	  KernelGenConProbMat<<<BlocksPerGrid, ThreadsPerBlock>>>(dev_conVecPtrLR,i,maxNeurons,nbN,Cpt,host_SigmaLR,host_DijLR) ; 	
	...
	KernelGenConProbMat2D<<<BlocksPerGrid, ThreadsPerBlock>>>(dev_conVecPtr,i,maxNeurons,nbN,Cpt,host_Sigma) ; 
      ...
      if(IF_LONG_RANGE)
	KernelConProbPreFactor<<<BlocksPerGrid, ThreadsPerBlock>>>(dev_conVecPtrLR, dev_preFactorLR, i, maxNeurons) ; 
	...
	KernelGenConProbMat2D<<<BlocksPerGrid, ThreadsPerBlock>>>(dev_conVecPtr,i,maxNeurons,nbN,Cpt,host_Sigma) ; 
      ...
      if(IF_LONG_RANGE) {
	KernelConProbNorm<<<BlocksPerGrid, ThreadsPerBlock>>>(dev_conVecPtrLR, dev_preFactorLR, i, maxNeurons) ; 
	KernelSumConVec<<<BlocksPerGrid, ThreadsPerBlock>>>(dev_conVecPtr, dev_conVecPtrLR, i, maxNeurons, host_DijLR) ;
      }
  ...
  if(IF_LONG_RANGE) {
    cudaFree(dev_conVecPtrLR); 
    cudaFree(dev_preFactorLR); 
    cudaFreeHost(host_SigmaLR); 
  }
